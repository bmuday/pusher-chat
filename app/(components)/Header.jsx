...
export default function Header () {
	...
	<div className="container flex justify-between h-16 mx-auto">
		...
		{headerLinks.map((link, index) => {
			const linkClass = classNames('flex items-center px-4 -mb-1 border-b-2 border-transparent', { "text-violet-600 border-violet-600" : pathname === link.url })
			console.log("link", link.label, link.url)
			return (
				<li key={index} className="flex">
				<Link rel="noopener noreferrer" href={link.url} className={linkClass}>{link.label}</Link>
			</li>
			)
		} )}
		</ul>
